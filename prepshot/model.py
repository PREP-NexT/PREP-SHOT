...
def create_model(para):
    ...
    # Define sets
    model.year = Set(initialize=para['year'],
                     ordered=True, doc='Set of planned timesteps')
    model.zone = Set(initialize=para['zone'],
                     ordered=True, doc='Set of zones')
    model.tech = Set(initialize=para['tech'],
                     ordered=True, doc='Set of technologies')
    model.hour = Set(initialize=para['hour'],
                     ordered=True, doc='Set of operation timesteps')
    model.hour_p = Set(initialize=[0]+para['hour'], ordered=True,
                       doc='Set of operation timesteps')
    model.month = Set(
        initialize=para['month'], ordered=True, doc='Set of planning timesteps')
    ...
    model.year_zone_zone_tuples = Set(initialize=[(y, z, z1) for y in model.year for z in model.zone for z1 in model.zone if
                                                  (z, z1) in para['transline'].keys()])
    model.hour_month_tech_tuples = model.hour * model.month * model.tech
    model.hour_month_year_zone_tech_tuples = model.hour * \
        model.month * model.year * model.zone * model.tech
    model.hour_p_month_year_zone_tuples = model.hour_p * \
        model.month * model.year * model.zone
    model.hour_month_year_zone_tuples = model.hour * \
        model.month * model.year * model.zone
    ...
    model.hour_p_month_year_zone_tech_tuples = model.hour_p * \
        model.month * model.year * model.zone * model.tech

    # Create sets by type of technology.
    if 'storage' in para['type'].values():
        model.storage_tech = Set(initialize=[i for i, j in para['type'].items(
        ) if j == 'storage'], ordered=True, doc='Set of storage technology')
    else:
        model.storage_tech = 0

    if 'nondispatchable' in para['type'].values():
        model.nondispatchable_tech = Set(initialize=[i for i, j in para['type'].items(
        ) if j == 'nondispatchable'], ordered=True, doc='Set of nondispatchable technology')
    else:
        model.nondispatchable_tech = 0

    if 'dispatchable' in para['type'].values():
        model.dispatchable_tech = Set(initialize=[i for i, j in para['type'].items(
        ) if j == 'dispatchable'], ordered=True, doc='Set of dispatchable technology')
    else:
        model.dispatchable_tech = 0

    if 'hydro' in para['type'].values():
        model.hydro_tech = Set(initialize=[i for i, j in para['type'].items(
        ) if j == 'hydro'], ordered=True, doc='Set of hydro technology')
    else:
        model.hydro_tech = 0

    model.hour_month_year_zone_storage_tuples = model.hour * \
        model.month * model.year * model.zone * model.storage_tech
    model.month_year_zone_storage_tuples = model.month * \
        model.year * model.zone * model.storage_tech
    model.hour_p_month_year_zone_storage_tuples = model.hour_p * \
        model.month * model.year * model.zone * model.storage_tech

    model.hour_month_year_zone_nondispatchable_tuples = model.hour * \
        model.month * model.year * model.zone * model.nondispatchable_tech

    # Define decision variables and intermediate variables.
    model.cost = Var(within=NonNegativeReals, doc='total cost of system [RMB]')
    model.cost_var = Var(within=NonNegativeReals,
                         doc='Variable O&M costs [RMB]')
    model.cost_newtech = Var(within=NonNegativeReals,
                             doc='Investment costs of new technology [RMB]')
    model.cost_fix = Var(within=NonNegativeReals,
                         doc='Fixed O&M costs [RMB/MW/year]')
    model.cost_newline = Var(
        within=NonNegativeReals, doc='Investment costs of new transmission lines [RMB]')
    model.income = Var(within=NonNegativeReals,
                       doc='total income of withdraw water [RMB]')
    ...
    model.total_cost_cons = Constraint(rule=cost_rule,
                                           doc='System total cost')
    model.total_cost = Objective(
        expr=model.cost, sense=minimize, doc='Minimize the sum of all cost types)')
    ...
    def var_cost_rule(model):
        ...
        """
        var_OM_tech_cost = sum([para['technology_variable_cost'][te, y] * model.gen[h, m, y, z, te] * para['dt'] *
                                para['var_factor'][y]
                                for h, m, y, z, te in model.hour_month_year_zone_tech_tuples])/para['weight']
        fuel_cost = sum([para['fuel_price'][te, y] * model.gen[h, m, y, z, te] * para['dt'] *
                         para['var_factor'][y]
                         for h, m, y, z, te in model.hour_month_year_zone_tech_tuples])/para['weight']
        var_OM_line_cost = 0.5 * sum([para['transline_variable_cost'][z, z1] * model.trans_export[h, m, y, z, z1] *
                                      para['var_factor'][y]
    ...
    def newtech_cost_rule(model):
        ...
        """
        return model.cost_newtech == sum(para['technology_investment_cost'][te, y] * model.cap_newtech[y, z, te] *
    ...
    def newline_cost_rule(model):
        ...
        """
        return model.cost_newline == 0.5 * sum(para['transline_investment_cost'][z, z1] * model.cap_newline[y, z, z1] *
                                               para['distance'][z, z1] *
    ...
    def fix_cost_rule(model):
        ...
        """
        fix_cost_tech = sum(para['technology_fix_cost'][te, y] * model.cap_existing[y, z, te]
                            * para['fix_factor'][y]
                            for y, z, te in model.year_zone_tech_tuples)
        fix_cost_line = 0.5 * sum(para['transline_fix_cost'][z, z1] * model.cap_lines_existing[y, z, z1]
                                  * para['fix_factor'][y]
    ...
    def emission_calc_by_zone_rule(model, y, z):
        return model.carbon_capacity[y, z] == sum(para['carbon_content'][te, y] *
                                                  model.gen[h, m, y,
                                                            z, te] * para['dt']
                                                  for h, m, te in model.hour_month_tech_tuples)
    ...
    def power_balance_rule(model, h, m, y, z):
        ...
        """
        imp_z = sum([model.trans_import[h, m, y, z1, z] for z1 in model.zone if
                     (z, z1) in para['transline'].keys()])
        exp_z = sum([model.trans_export[h, m, y, z, z1] for z1 in model.zone if
                     (z, z1) in para['transline'].keys()])
        gen_z = sum([model.gen[h, m, y, z, te] for te in model.tech])
        charge_z = sum([model.charge[h, m, y, z, te]
    ...
    if model.storage_tech != 0:
        def energy_storage_balance_rule(model, h, m, y, z, te):
            ...
            return (model.storage[h, m, y, z, te] == model.storage[h-1, m, y, z, te] -
                    model.gen[h, m, y, z, te] * para['efficiency_out'][te, y]*para['dt'] +
                    model.charge[h, m, y, z, te] * para['efficiency_in'][te, y]*para['dt'])

        def init_energy_storage_rule(model, m, y, z, te):
            return (model.storage[0, m, y, z, te] == para['init_storage_level'][te, z] *
                    model.cap_existing[y, z, te] * para['energy_power_ratio'][te])
   ...
   #  add constarints
    model.cost_var_cons = Constraint(
        rule=var_cost_rule, doc='Variable O&M cost and fuel cost')
    model.newtech_cost_cons = Constraint(
        rule=newtech_cost_rule, doc='Investment costs of new technology')
...
def add_hydro(model, para):
    ...
    Hour = para['hour']
    Hour_period = [0] + para['hour']  # storage

    model.station = Set(
        initialize=para['stcd'], ordered=True, doc='Set of hydropower plants')

    model.station_hour_month_year_tuples = model.station * \
        model.hour * model.month * model.year
    model.station_month_year_tuples = model.station * model.month * model.year
    model.station_hour_p_month_year_tuples = model.station * \
        model.hour_p * model.month * model.year
    ...
    model.inflow = Var(model.station_hour_month_year_tuples,
                       within=Reals, doc='inflow of reservoir [m3/s]')
    model.outflow = Var(model.station_hour_month_year_tuples,
                        within=NonNegativeReals, doc='inflow of reservoir [m3/s]')
    ...
    def total_inflow_rule(model, s, h, m, y):
        ...
        for ups, delay in zip(para['connect'][para['connect']['NEXTPOWER_ID'] == s].POWER_ID, para['connect'][para['connect']['NEXTPOWER_ID'] == s].delay):
            ...
            if (h - delay >= Hour[0]):
                ...
                # up_stream_outflow += 0
                up_stream_outflow += model.outflow[ups,
                                                   Hour[-1] - delay + h, m, y]
